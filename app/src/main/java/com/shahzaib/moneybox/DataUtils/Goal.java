package com.shahzaib.moneybox.DataUtils;
...
public class Goal {
    ...
    //TODO: Get the  money symbol (PKR) and also assign with the all amounts
...
//        }
    ...
    public String getSavingNeeded() {
        ...
        switch (savingFrequency)
        {
            case DAILY:
                if(remainingDays()==0){
                    return Goal.separateNumberWithComma(getRemainingAmount())+" /-Daily";
                }
                else {
                    double result =  ((double)getRemainingAmount()  / (double)remainingDays()) ;
                    return Goal.separateNumberWithComma(result)  +" /-Daily";
                }

            case WEEKLY:
                if(getRemainingWeeks()==0){
                    return Goal.separateNumberWithComma(getRemainingAmount())+" /-Weekly";
                }
                else
                {
                    double result =  ((double)getRemainingAmount()  / (double)getRemainingWeeks()) ;
                    return Goal.separateNumberWithComma(result)  +" /-Weekly";
                }

            case MONTHLY:
                if(getRemainingMonths()==0){
                    return Goal.separateNumberWithComma(getRemainingAmount())+" /-Monthly";
                }
                else
                {
                    double result =  ((double)getRemainingAmount()  / (double)getRemainingMonths()) ;
                    return Goal.separateNumberWithComma(result)  +" /-Monthly";
                }
    ...
    public String getPercentageCompleted_With_DepositedAmount() {
        ...
        {
            return ""+getPercentageCompleted()+"% ("+Goal.separateNumberWithComma(getDepositedAmount())+")";
        }
        else
        {
